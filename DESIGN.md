...
Release publication is package-first. Staging publishes only the exact signed MSI, checksum, license, and notices, with no appcast asset, so the deployed helper can mirror and validate the candidate at a direct `.msi` URL without changing either live feed. Each stage invocation adds a fresh hidden deployment marker to the release notes so an interrupted mirror can be retriggered even when the package assets are already exact. Version 1.0.7 is the immutable legacy bridge for clients through v1.0.6; later releases carry that exact signed pair as GitHub-latest compatibility assets but never regenerate or advance it. Only after the direct MSI and both prompted and silent LocalSystem upgrades from the immediately preceding published client pass may finalization upload the frozen legacy assets followed by the candidate v2 signature and `appcast-v2.xml`. The v2 XML is the rollout point. The subsequent release edit makes the deployed helper advance `updates.resticpal.com/appcast-v2.xml` while revalidating and preserving the pinned legacy bytes.
...
- Qualify LocalSystem source access, ProgramData ACL behavior, VSS, repair, same-version and additional major upgrades, and reboot/restart cases across supported Windows 10/11 builds. The published-client v1.0.4-to-v1.0.5 updater lifecycle, direct-MSI v1.0.6-to-v1.0.7 major upgrade, and signed v1.0.6-to-v1.0.7 prompted and automatic update paths pass on Sandbox build 26100. Every later release still requires prompted and automatic qualification from its immediately preceding published client.
