...
The launcher blocks until the guest reports a result, making it suitable for agentic shell work. Each run gets a directory under `artifacts\windows-sandbox`. That directory contains the generated `.wsb` configuration, a copy of the tested MSI, `result.json`, a guest transcript, and the MSI lifecycle logs. Failed installer runs also export relevant Windows events and service-startup probes.
